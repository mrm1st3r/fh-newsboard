...
class DocumentWriter extends DocumentStubWriter {
    ...
    private void writeClassifications(XMLStreamWriter writer) throws XMLStreamException {
        ...
        for (Classification c : collectedClassifications) {
            ...
            if (c.getDocumentId() != null) writer.writeAttribute("documentid", Integer.toString(c.getDocumentId()));
            if (c.getSentenceId() != null) writer.writeAttribute("sentenceid", Integer.toString(c.getSentenceId()));
            if (c.getConfidence() != 0) writer.writeAttribute("confidence", Double.toString(c.getConfidence()));
